...
vec2 house(vec3 p) 
{
	float res = sdBox(p - vec3(0, 10, 0), vec3(2, 10, 2)); 
	return vec2(res, MAT_HOUSE);
}

vec2 scene(vec3 p, vec3 rd)
{
	vec2 res = morph(p - vec3(0,2,0));
...
void main()
{
    ...
    vec3 eye = vec3(10*cos(tick/5.0), 20, 10*sin(tick/5.0));
	//vec3 eye = vec3(0, 2, 6);
	vec3 tar = vec3(0, 0, 0);
    	 ...
    	 for(int i = 0; i < 1000 && t < 1000; ++i)
	        ...
	        if(d < 0.01)
	        {
				...
				if (m == MAT_MIRROR) {
	        		...
	        		color = mix(color, vec3(0.01,0.03,0.3) * (1.0 + diffuse), ref); 
	        		color += ref*vec3(specular(normal, -invLight, normalize(eye - p), 70.0));
	        	} else if (m == MAT_HOUSE) {
	        		vec3 cw = mix(vec3(0.5), vec3(texture(noiseP, vec2(p.x + p.z, p.y)).x), 0.5);
	        		vec3 c = mod(p.y, 2) >= 1.0 ? cw : (mod(p.x, 0.3) >= 0.25 ? cw : (mod(p.z, 0.3) >= 0.25 ? cw : vec3(0.1,0.1,0.2)));
	        		//c = mod(p.x, 0.3) >= 0.25 ? vec3(0.5) : c;
	        		//c = mod(p.z, 0.3) >= 0.25 ? vec3(0.5) : c;
	            ...
	            if (m == MAT_MIRROR) {
	        		...
	        		ref *= mod(p.y, 2) >= 1.0 ? 0.0 : (mod(p.x, 0.3) >= 0.25 ? 0.0 : (mod(p.z, 0.3) >= 0.25 ? 0.0 : 0.5));
	        	}
	           	break;
	        }
